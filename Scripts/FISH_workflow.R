...
library(tidyverse)
library(sf)
library(raster) # this might cause issues with select() in dplyr so be sure to usee dplyr::select() if using the function

source("../restoration_runs/Scripts/FISH_map_functions.R")
...
cum_fish_by_date <- fish %>%
  group_by(DATE)%>%
  summarise(AA = mean(depth_AA_TOTFISH_CUM),
            BB = mean(depth_BB_TOTFISH_CUM),
            CC = mean(depth_CC_TOTFISH_CUM),
            DD = mean(depth_DD_TOTFISH_CUM),
            EE1 = mean(depth_EE1_TOTFISH_CUM),
            EE2 = mean(depth_EE2_TOTFISH_CUM),
            ECBr = mean(depth_ECBr_TOTFISH_CUM),
            NA25 = mean(depth_NA25_TOTFISH_CUM))
...
cum_fish_plot <- ggplot(cum_fish_by_date) + 
  geom_line(aes(x=DATE, y=totfish_cum, colour=Scenario), stat="identity", size=0.5)+ 
