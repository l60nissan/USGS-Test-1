...
MarlProcess <- function(base_file,
  ...
  mp <- mp[, !names(mp) %in% c("ID", "hsi_MaxDry", "hsi_Hydrop",
                               "hsi_WDepth", "hsi_DDepth", "hsi_CScore",
                               "RSM_ID")]
  ...
  return(list("ind_df" = ind_df, "diff_df" = diff_df,
              "full_df" = mp_diff, "diff_name" = diff_name, 
              "alt_name" = alt_name, "base_name" = base_name,
              "acreage_df" = acreage_diff_df, "Map_title" = marl_title, 
              "sp_string" = marl_string))
}      
